...
- **missing-security-headers** — CSP, HSTS, X-Content-Type-Options,
  ...
  response. (Mirrors our own Deploy Guard `--advisory headers` philosophy —
  this is the same rule NightShift can now enforce from the inside.)
...
- **P5** — nightloop integration + Deploy Guard note.
