...
import { enumerateElements, collectCandidates, locatorKey, buildLocator, CANDIDATE_SELECTOR } from "./elements.mjs";
import { normalizeName, locatorToSelector, elementSignature, signatureKey } from "./census.mjs";
...
// Sentinels around the embedded census + enumeration functions (parity-tested).
export const EMBED_CENSUS_BEGIN = "// --- begin embedded lib/census.mjs + lib/elements.mjs (do not edit; parity-tested) ---";
export const EMBED_CENSUS_END = "// --- end embedded lib/census.mjs + lib/elements.mjs ---";
...
export function generateReproScript(finding, config) {
  ...
  if (finding.source === "oracle:expected-element") {
    return generateCensusReproScript(finding, config);
  }
...
// Expected-element repro: goto the route, resize to the viewport class,
// re-enumerate (the REAL lib/elements.mjs enumeration, embedded byte-identical),
// and exit 0 while the baseline element is missing/invisible, 1 once restored.
// No trace replay and no oracle signatures — the disappearance is the claim.
export function generateCensusReproScript(finding, config) {
  const census = finding && finding.census;
  if (!census || !census.route || !census.viewport || !census.element) {
    throw new Error("generateCensusReproScript: finding " + String(finding && finding.id) + " has no census payload");
  }
  const js = (value) => JSON.stringify(value ?? null, null, 2);
  const oneLine = (value) => String(value ?? "").replace(/\s+/g, " ").trim();
  const navTimeoutMs = Number.isInteger(config?.reverify?.navTimeoutMs) ? config.reverify.navTimeoutMs : 15000;
  const targetKey = signatureKey(census.element);
  const max = Number.isFinite(census.max) ? census.max : null; // null -> Infinity at runtime
  const ignoreSelectors = Array.isArray(census.ignoreSelectors) ? census.ignoreSelectors : [];

  return `#!/usr/bin/env node
// NightShift QA repro script — ${oneLine(finding.id)}: ${oneLine(finding.title)}
// Standalone: imports only playwright. Navigates to the route, resizes to the
// ${oneLine(census.viewport.name)} viewport class, re-enumerates the interactive elements with the
// LLM out of the loop, and checks whether the baseline element is still there.
// Exit 0 = element still missing/invisible (bug reproduced), exit 1 = restored.
import { chromium } from "playwright";

const FINDING_ID = ${js(finding.id)};
const ROUTE = ${js(census.route)};
const VIEWPORT = ${js(census.viewport)};
const TARGET_KEY = ${js(targetKey)};
const IGNORE_SELECTORS = ${js(ignoreSelectors)};
const MAX = ${js(max)};
const NAV_TIMEOUT_MS = ${navTimeoutMs};
const SETTLE_CAP_MS = 1500; // mirrors lib/trace.mjs

${EMBED_CENSUS_BEGIN}
const CANDIDATE_SELECTOR = ${js(CANDIDATE_SELECTOR)};

${fnSource(enumerateElements)}

${fnSource(locatorKey)}

${fnSource(buildLocator)}

${fnSource(collectCandidates)}

${fnSource(normalizeName)}

${fnSource(locatorToSelector)}

${fnSource(elementSignature)}

${fnSource(signatureKey)}
${EMBED_CENSUS_END}
...
async function main() {
  ...
  try {
    ...
    let navigated = true;
    try {
      await page.goto(ROUTE, { timeout: NAV_TIMEOUT_MS, waitUntil: "domcontentloaded" });
    } catch {
      navigated = false;
    }
    try {
    ...
    let present = false;
    if (navigated) {
      const els = await enumerateElements(page, MAX === null ? Infinity : MAX, IGNORE_SELECTORS);
      const keys = new Set(els.map((e) => signatureKey(elementSignature(e))));
      present = keys.has(TARGET_KEY);
    } else {
      console.error("could not navigate to " + ROUTE + " — cannot evaluate the census");
    }

    // Reproduced = the baseline element is missing/invisible at this viewport.
    const reproduced = navigated && !present;
    console.log((reproduced ? "REPRODUCED " : "NOT REPRODUCED ") + FINDING_ID + " expected-element@" + VIEWPORT.name + " " + TARGET_KEY);
    process.exitCode = reproduced ? 0 : 1;
  } finally {
    await browser.close().catch(() => {});
  }
