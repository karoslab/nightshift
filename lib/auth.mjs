...
// for the roles that logged in. Returns [] when no auth is configured
// (anonymous-only). This is the single per-role login loop the session uses:
// a role whose login THROWS is skipped with an error log (never discarding the
// other roles or the implicit anonymous pass the caller prepends), and a role
// that logs in but yields an empty storageState is kept with a warning so the
// run still explores it.
export async function establishRoleSessions({ config, browser, runDir, env = process.env, log = () => {} }) {
  ...
  for (const role of roles) {
    ...
    let storageState;
    try {
      ({ storageState } = await loginRole({ browser, role, origin, env, navTimeoutMs, log }));
    } catch (e) {
