...
import { loginRole, saveRoleState, storageStateIsEmpty } from "./auth.mjs";
...
export async function runSession({ config, brain, runDir, log = defaultLog, mintId = createIdMinter(), env = process.env }) {
  ...
  const allFindings = [];
  try {
    // Anonymous (implicit default) always runs; each auth role that logs in
    // adds a pass with its persisted storageState. A role whose login fails is
    // skipped with a warning so it can't discard the anonymous pass.
    const roleSessions = [{ name: ANONYMOUS_ROLE, storageState: undefined }];
    for (const role of authRoles) {
      try {
        const { storageState } = await loginRole({ browser, role, origin, env, navTimeoutMs, log });
        saveRoleState(runDir, role.name, storageState);
        if (storageStateIsEmpty(storageState)) {
          log("warn", `auth: role "${role.name}" produced an empty storageState — exploring it anyway, but login may have failed`);
        } else {
          log("info", `auth: role "${role.name}" authenticated (${storageState.cookies?.length ?? 0} cookie(s))`);
        }
        roleSessions.push({ name: role.name, storageState });
      } catch (e) {
        log("error", `auth: role "${role.name}" login failed — skipping this role: ${String(e?.message ?? e)}`);
      }
    }
...
async function exploreRole(browser, roleSession, { config, brain, runDir, log, mintId, stats, journeys, origin, navTimeoutMs }) {
  ...
  try {
    ...
    // are NOT gated by the LLM budget (they make no brain calls).
    await runJourneys(page, { journeys, origin, collector, oracles, navTimeoutMs, log });
  } finally {
