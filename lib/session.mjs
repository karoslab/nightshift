...
// Divide the per-session budget caps evenly across `roleCount` roles so the
// TOTAL across all roles stays within the user's configured maxLlmCalls /
// maxMinutes. Exported for the budget test. roleCount is always >= 1 (anonymous
// always runs), so with no auth roles this returns the config caps unchanged.
// Each cap floors to at least 1 so a small config never zeroes out a role.
export function subdivideBudget(budgetConfig = {}, roleCount = 1) {
  const n = Math.max(1, roleCount);
  const split = (value, fallback) => Math.max(1, Math.floor((value ?? fallback) / n));
  return {
    ...budgetConfig,
    maxLlmCalls: split(budgetConfig.maxLlmCalls, 40),
    maxMinutes: split(budgetConfig.maxMinutes, 45),
  };
}
...
export async function runSession({ config, brain, runDir, log = defaultLog, mintId = createIdMinter(), env = process.env }) {
  ...
  try {
    ...
    // establishRoleSessions owns the per-role login loop, already skips a role
    // whose login fails (logged), and returns [] when no auth is configured —
    // so it can never discard the anonymous pass prepended here.
    const authSessions = await establishRoleSessions({ config, browser, runDir, env, log });
    const roleSessions = [{ name: ANONYMOUS_ROLE, storageState: undefined }, ...authSessions];

    // The configured budget (maxLlmCalls / maxMinutes) governs the WHOLE
    // session, not each role — otherwise N roles would silently multiply the
    // user's Claude spend cap by N. Split it evenly across the roles so the
    // total stays within the configured cap and no role starves a later one.
    const roleBudget = subdivideBudget(config.budget, roleSessions.length);
    if (roleSessions.length > 1) {
      log(
        "info",
        `budget: split across ${roleSessions.length} role(s) — each gets ` +
          `${roleBudget.maxLlmCalls} LLM call(s) / ${roleBudget.maxMinutes} min (total stays within the configured cap)`,
      );
    }

    for (const roleSession of roleSessions) {
      // A fresh budget instance per role (fresh timer at the start of that
      // role's pass), all sharing the divided per-role caps.
      const budget = createSessionBudget(roleBudget);
...
async function exploreRole(browser, roleSession, { config, brain, runDir, log, mintId, stats, journeys, origin, navTimeoutMs, budget }) {
