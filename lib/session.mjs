...
export async function runSession({ config, brain, runDir, log = defaultLog }) {
  ...
  const navAllowList = new Set(seeds.map((u) => new URL(u).pathname));
  ...
  const collector = createFindingCollector({ oracles, consoleTail, runDir, log });
  ...
  const visitRoute = async (page, route) => {
    ...
    for (let n = 0; n < actionsPerPage; n++) {
      ...
      if (json.action && typeof json.action === "object") {
        ...
        const step = await executeAction(page, json.action, elements);
        step.i = stepIndex;
        trace.push(step);
    ...
    // late listeners (settle already waited, but drain anything that landed)
    await collector.collectOracleFindings(page, trace);
    return true;
  };
...
function createFindingCollector({ oracles, consoleTail, runDir, log }) {
  const findings = [];
  const seenSignatures = new Set();
  let consumedEvents = 0;
  let nextId = 1;
  const mintId = () => "NS-" + String(nextId++).padStart(3, "0");
