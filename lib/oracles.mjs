...
// Framework prefetch fetches (Next.js App Router router-prefetch, browser
// speculative "purpose"/"sec-purpose" prefetch) get cancelled on navigation
// like any other in-flight request, but Chromium sometimes reports the
// cancellation as net::ERR_FAILED instead of net::ERR_ABORTED — and it
// re-reproduces on replay because the repro replays the same navigation.
// The header, not the error text, is the only reliable discriminator: a
// genuinely failing same-origin fetch (server crash, CORS misconfig) must
// still file even if it happens to say ERR_FAILED.
export function isPrefetchRequest(request) {
  const headers = request.headers();
  if (Object.prototype.hasOwnProperty.call(headers, "next-router-prefetch")) return true;
  if (/^prefetch/i.test(headers["purpose"] || "")) return true;
  if (/^prefetch/i.test(headers["sec-purpose"] || "")) return true;
  return false;
}
