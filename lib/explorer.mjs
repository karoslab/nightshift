...
// Canonical action verbs executeAction understands — the only values
// denyActionKinds can meaningfully name. Exported so config.mjs validates
// against the same set instead of drifting out of sync.
export const ACTION_KINDS = ["click", "fill", "select", "press", "goto", "back"];
...
export async function executeAction(page, action, elements, opts = {}) {
  ...
  const denyActionKinds = Array.isArray(opts.denyActionKinds) ? opts.denyActionKinds : [];
  if (kind && denyActionKinds.includes(kind)) {
    return unexecutedStep(page, action, "blocked by denyActionKinds: " + kind);
  }
  try {
