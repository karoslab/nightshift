...
function requireRoutes(value, key) {
...
function validate(config) {
  for (const section of ["target", "brain", "budget", "oracles", "reverify", "report"]) {
  ...
  requireRoutes(target.routes, "target.routes");
  ...
  if (!Number.isInteger(budget.stopAtHour) || budget.stopAtHour < 0 || budget.stopAtHour > 23) {
    fail(`budget.stopAtHour must be an integer hour of day (0-23) — got ${got(budget.stopAtHour)}`);
  }
