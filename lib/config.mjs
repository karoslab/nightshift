...
const DEFAULTS = Object.freeze({
  ...
  oracles: {
    ...
    // Element-census baseline oracle (structural visual-regression detection).
    // Off by default: existing users and the nightloop opt in per target, and a
    // disabled oracle is byte-identical to today (it never runs a census).
    expectedElements: {
      enabled: false,
      ignoreSelectors: [],
    },
...
function validate(config) {
  ...
  const expectedElements = oracles.expectedElements;
  if (!isPlainObject(expectedElements)) {
    fail(`oracles.expectedElements must be a JSON object with "enabled" and "ignoreSelectors" — got ${got(expectedElements)}`);
  }
  if (typeof expectedElements.enabled !== "boolean") {
    fail(`oracles.expectedElements.enabled must be a boolean (default false) — got ${got(expectedElements.enabled)}`);
  }
  if (
    !Array.isArray(expectedElements.ignoreSelectors) ||
    !expectedElements.ignoreSelectors.every((s) => typeof s === "string")
  ) {
    fail(`oracles.expectedElements.ignoreSelectors must be an array of CSS selector strings — got ${got(expectedElements.ignoreSelectors)}`);
  }
  for (const selector of expectedElements.ignoreSelectors) {
    if (!isStructurallyValidSelector(selector)) {
      fail(`oracles.expectedElements.ignoreSelectors entry is not valid CSS (unbalanced brackets/quotes or empty) — got ${got(selector)}`);
    }
  }
