...
export async function runSweep({ config, brain = null, runDir, log = defaultLog, mintId = createIdMinter(), budget }) {
  ...
  // Persist the FULL resumable queue after every element and every route: the
  // remaining frontier (current route already shifted off), the enqueued set,
  // and the harvested navAllowList — without these a resume cannot reconstruct
  // what was left to sweep.
