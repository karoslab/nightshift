...
import { collectCensus } from "./census.mjs";
import { baselinesDir, beginBaselineRun, recordRouteCensus } from "./baseline.mjs";
...
export async function runSweep({ config, brain = null, runDir, log = defaultLog, mintId, budget, updateBaselines = false }) {
  ...
  // Queue state. Fresh run: seed from config. Resume: restore the PERSISTED
  // queue — the frontier (harvested-but-unvisited routes), the enqueued set,
  // and the navAllowList — because rebuilding from seeds alone would lose every
  // discovered route the moment the seed is marked done, and the interrupted
  // route (already shifted off the frontier when it began) would never run
  // again. That is requirement #7: an interrupted sweep CONTINUES, not restarts.
  const frontier = resuming ? [...(checkpoint.frontier ?? [])] : [...new Set(seeds)];
  const enqueued = new Set(resuming ? (checkpoint.enqueued ?? frontier) : frontier);
  const navAllowList = new Set(
    resuming ? (checkpoint.navAllowList ?? []) : seeds.map((u) => pathSearchOf(u)),
  );
  const routesDone = new Set(resuming ? (checkpoint.routesDone ?? []) : []);
  const current = resuming ? (checkpoint.current ?? null) : null;

  // Findings discovered before the interruption. They are re-collected into the
  // resumed run's collector (deduped by signature) so a bug on a route that
  // completed before the stop survives into the final report instead of being
  // silently overwritten — requirement #5 (a finding survives) and #7 (continue).
  const restoredFindings = resuming ? (checkpoint.findings ?? []) : [];
  // Continue minting ids AFTER the restored ones so a resume never re-mints an
  // id already used (which would collide screenshots/repro paths).
  mintId = mintId ?? createIdMinter(nextIdAfter(restoredFindings));

  const coverage = createCoverageTracker();
  if (resuming) for (const r of checkpoint.coverage?.routes ?? []) coverage.restore(r);

  if (resuming) {
    // A seed added to config between runs that we have not seen yet.
    for (const seed of new Set(seeds)) {
      if (!routesDone.has(seed) && !enqueued.has(seed)) {
        enqueued.add(seed);
        frontier.push(seed);
      }
    }
    // Re-enqueue the interrupted route at the front so it resumes FIRST at its
    // saved element offset (it was shifted off the frontier when it began).
    if (current?.route && !routesDone.has(current.route) && !frontier.includes(current.route)) {
      frontier.unshift(current.route);
    }
  }
  ...
  const collector = createFindingCollector({ oracles, consoleTail, runDir, log, mintId, initialFindings: restoredFindings });

  // Expected-element census (opt-in). Disabled -> this whole block is skipped
  // and the sweep is byte-identical to today (no resize, no baseline I/O).
  const expectedElements = config.oracles?.expectedElements ?? {};
  const censusEnabled = expectedElements.enabled === true;
  let censusCtx = null;
  if (censusEnabled) {
    const dir = baselinesDir(config);
    const runId = path.basename(runDir);
    const { configHash, reseedAll } = beginBaselineRun({ dir, config, runId, updateBaselines });
    censusCtx = { dir, runId, configHash, reseedAll, ignoreSelectors: expectedElements.ignoreSelectors ?? [] };
    if (reseedAll) log("info", "expected-element census: seeding baselines this run (first run, config change, or --update-baselines)");
  }

  // One route's census: resize to each viewport class, re-enumerate (reusing
  // lib/elements.mjs), diff against the stored baseline, and file a candidate
  // for each disappearance. Best-effort — a census failure must never sink the
  // sweep. Restores the page viewport afterward so element/form actions run at
  // the original size (keeping enabled/disabled sweeps otherwise aligned).
  const runRouteCensus = async (page, route) => {
    if (!censusCtx) return;
    let prev = null;
    try {
      prev = page.viewportSize();
      const census = await collectCensus(page, { ignoreSelectors: censusCtx.ignoreSelectors });
      const res = recordRouteCensus({
        dir: censusCtx.dir,
        route,
        census,
        runId: censusCtx.runId,
        configHash: censusCtx.configHash,
        reseedAll: censusCtx.reseedAll,
      });
      if (!res.seeded && res.missing.length) {
        await collector.collectCensusFindings(page, res.missing, { route, ignoreSelectors: censusCtx.ignoreSelectors });
      }
    } catch (err) {
      log("warn", "expected-element census failed on " + route + ": " + String(err && err.message ? err.message : err));
    } finally {
      if (prev) {
        try {
          await page.setViewportSize(prev);
        } catch {
          // page/context gone — the crawl loop will handle it
        }
      }
    }
  };
  ...
  // Persist the FULL resumable state after every element and every route: the
  // remaining frontier (current route already shifted off), the enqueued set,
  // the harvested navAllowList, AND the candidate findings collected so far —
  // without the findings a resume would overwrite the report and silently drop
  // every bug found before the interruption.
  ...
  const sweepRoute = async (page, route, startIndex) => {
    ...
    // Census once per route, at its first visit this run (a resumed mid-route
    // sweep already recorded the census before the interruption). Census only
    // resizes + enumerates — it never navigates — so the element queue below
    // still resolves against this route.
    if (startIndex === 0) await runRouteCensus(page, route);
  ...
  try {
    ...
    while (frontier.length > 0 && visited.length < maxRoutes && budget.timeLeft()) {
      ...
      const startIndex = current && current.route === route ? current.elementsDone : 0;
  ...
  for (let fi = 0; fi < forms.length; fi++) {
    for (const pass of SWEEP_PASSES) {
      ...
      for (const field of form.fields) {
        // A denylisted field was already counted once in the route-start
        // deniedByDenylist tally (enumerateElements includes form inputs), so
        // skip it here WITHOUT re-recording — counting it again per form pass
        // (3x) would inflate the Skipped column.
        if (field.denied) continue;
...
// The next NS-nnn number to mint, given the already-restored findings: one past
// the highest existing id so a resumed run never re-mints (and overwrites) one.
function nextIdAfter(findings) {
  let max = 0;
  for (const f of findings ?? []) {
    const m = /^NS-(\d+)$/.exec((f && f.id) || "");
