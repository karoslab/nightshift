...
import { signatureKey, VIEWPORT_CLASSES } from "./census.mjs";
...
export function createFindingCollector({ oracles, consoleTail, runDir, log, mintId, initialFindings = [], tag = {} }) {
  ...
  // Element-census disappearances (lib/census.mjs diff): each missing baseline
  // element becomes an `expected-element` candidate. Deduped by a census
  // pseudo-signature (route + viewport + element key) so a resumed sweep never
  // double-files the same disappearance. A screenshot is taken AT the missing
  // element's viewport class so the evidence shows the broken layout.
  const collectCensusFindings = async (page, missing, { route, ignoreSelectors = [], max = null } = {}) => {
    const created = [];
    for (const { viewport: viewportName, element } of missing) {
      const viewport = VIEWPORT_CLASSES.find((v) => v.name === viewportName) ?? { name: viewportName, width: 1280, height: 800 };
      const signature = "expected-element|" + pathnameOf(route) + "|" + viewport.name + "|" + signatureKey(element);
      if (seenSignatures.has(signature)) continue;
      seenSignatures.add(signature);
      const id = mintId();
      try {
        ...
        // page/context gone — snapshot() below degrades gracefully
      }
      const shot = await snapshot(page, id);
      const label = element.name ? `the ${element.role ?? element.tag} "${element.name}"` : `a ${element.role ?? element.tag}`;
      findings.push({
        id,
        ...stamp,
        source: "oracle:expected-element",
        title: `Expected element missing at ${viewport.name} — ${label}`.slice(0, 120),
        severity: "major",
        signature,
        failure: null,
        semantic: null,
        check: null,
        census: { route, viewport, element, ignoreSelectors, max },
        trace: [gotoStep(route)],
        evidence: { ...evidence(page, shot), viewport: viewport.name },
        status: "candidate",
        reverify: null,
      });
      created.push(id);
      log("info", "candidate " + id + " (expected-element): " + label + " missing at " + viewport.name);
    }
    return created;
  };

  return { findings, collectOracleFindings, collectSemanticFinding, collectCensusFindings };
}

// A minimal recorded goto step for an expected-element finding: the reverify
// and repro paths re-navigate to the route, resize, and re-enumerate — the
// trace is what the report renders as "steps to reproduce".
