...
import { captureEvidenceSnapshot } from "./evidence.mjs";
...
function findingToMd(f) {
  ...
  if (f.evidenceSnapshot) {
    lines.push(`- Evidence snapshot: ${f.evidenceSnapshot.dir}/snapshot.tar.gz (sha256 ${f.evidenceSnapshot.sha256})`);
  }
  lines.push("");
  return lines.join("\n");
}
...
// Local-only evidence for confirmed findings only ("confirmed" is the tier
// backed by a deterministic reproduced signature — see lib/reverify.mjs).
// One finding's capture failure must not sink the whole report.
function withEvidenceSnapshots(findings, runDir, repoDir, captureEvidence) {
  return findings.map((f) => {
    if (f.status !== "confirmed") return f;
    try {
      const evidenceDir = path.join(runDir, "evidence", f.id);
      const { manifest } = captureEvidence({ repoDir, evidenceDir });
      return { ...f, evidenceSnapshot: { dir: `evidence/${f.id}`, ...manifest } };
    } catch {
      return f;
    }
  });
...
export function writeReport(
  runDir,
  { config = {}, findings = [], stats = {}, brainMeta = {}, repoDir = process.cwd(), captureEvidence = captureEvidenceSnapshot } = {},
) {
  ...
  const finalFindings = withEvidenceSnapshots(withReproScripts(absRunDir, findings, config), absRunDir, repoDir, captureEvidence);
